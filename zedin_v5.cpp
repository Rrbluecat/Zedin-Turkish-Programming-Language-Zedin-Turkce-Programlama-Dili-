...
private:
    void registerBuiltins() {
        ...
        // oku() — kullanıcıdan girdi al (readline ile gecmis destegi)
        globals->define("oku", Val::makeNative([](vector<Val> args) -> Val {
            string prompt = "";
            if (!args.empty()) prompt = args[0].toString();
            char* line = readline(prompt.c_str());
            if (!line) return Val(string(""));
            string result(line);
            if (!result.empty()) add_history(line);
            free(line);
            return Val(result);
        }));
        ...
        globals->define("ekran_temizle", Val::makeNative([](vector<Val> args) -> Val {
            system("clear");
            return Val();
        }));
