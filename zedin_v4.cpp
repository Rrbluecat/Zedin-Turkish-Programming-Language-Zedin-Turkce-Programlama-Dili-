...
public:
    ...
    Val get(const Token& name) {
        ...
        throw ZedinError(name.pos, "Tanımsız değişken: '" + name.lex + "'");
    }
...
private:
    ...
    Val evaluate(Expr* e) {
        ...
        if (auto ex = dynamic_cast<IndexAssignExpr*>(e)) {
            ...
            int i = (int)idx.num;
            if (i < 0 || i >= (int)obj.list->elements.size())
                throw ZedinError(ex->bracket.pos, "İndeks sınır dışı.");
...
class Parser {
    ...
    Token consume(TType t, string msg) {
        ...
        throw ZedinError(current().pos, msg + " (Alınan: '" + current().lex + "')");
    }
...
void Interpreter::importFile(const string& path, SourcePos pos) {
    ifstream file(path);
    if (!file.is_open()) throw ZedinError(pos, "Modül bulunamadı: '" + path + "'");
    stringstream buf; buf << file.rdbuf();
    Lexer lex(buf.str(), path);
    auto toks = lex.scan();
...
public:
    BCompiler(const string& name = "<ana>") {
...
private:
    ...
    void derleVarOku(const string& ad) {
        ...
        if (idx >= 0) {
            ...
            // Global — sabit olarak adı yükle, VM global map'ten alır
            // Geçici çözüm: global değişkenleri sabit pool'a isim olarak ekle
            int sidx = chunk->sabitEkle(BVal(ad));
            chunk->yazOpArg(OpCode::OP_METIN, sidx, satirNo); // geçici
            // TODO: OP_YUKLE_GLOBAL opcode eklenecek
        }
    }
    ...
    void derleFonk(FuncDecl* st) {
        ...
        BCompiler subCompiler(st->name.lex);
